...
import { Loader2, AlertTriangle, CheckCircle, Rocket } from 'lucide-react';
...
export default function AbaMapa({ isVisible }) {
    ...
    // --- Estados da Rota/Veículo (REFATORADO) ---
    const { meusVeiculos, loading: veiculosLoading } = useVeiculos();
    const { salvarViagem, rotaParaCarregar, setRotaParaCarregar } = useViagens();

    // Agora salvamos apenas o ID (String) para evitar bugs de seleção
    const [selectedVehicleId, setSelectedVehicleId] = useState(null);

    // Derivamos o objeto veículo da lista original (garante dados sempre frescos)
    ...
    // ... (drawRoute e clearRoute mantidos iguais para economizar espaço, copie do anterior se precisar) ...
    ...
    const forceClearRoute = useCallback((map) => {
        ...
        try {
            ...
            setSelectedVehicleId(null); // Limpa o ID selecionado
    ...
    useEffect(() => {
        if (rotaParaCarregar && isVisible) {
            ...
            setDestination({
                ...
                address: "", // Mantém visualmente limpo
                displayName: rotaParaCarregar.apelido || "Destino"
            });
    ...
    useEffect(() => {
        ...
        originMarkerRef.current = new maplibregl.Marker({ element: el })
            ...
            .addTo(map);
        if (!destination) map.flyTo({ center: [origin.longitude, origin.latitude], zoom: 12, duration: 1500 });
    }, [origin, destination]);
    ...
    useEffect(() => {
        ...
        destinationMarkerRef.current = new maplibregl.Marker({ element: el })
            ...
            .addTo(map);
        if (!origin) map.flyTo({ center: [destination.longitude, destination.latitude], zoom: 12, duration: 1500 });
    }, [destination, origin]);
            ...
            const fetchAndDrawRoute = async () => {
                ...
                try {
                    ...
                    if (routeCache.has(cacheKey)) {
                        ...
                        if (!routes || routes.length === 0) throw new Error("Nenhuma rota encontrada.");
                    ...
                    map.fitBounds(bounds, { padding: { top: 50, bottom: 50, left: 50, right: 50 }, duration: 1500 });
                } catch (error) {
    ...
    const canStartTrip = origin && destination && selectedVehicle && totalDistance && autonomyReport && autonomyReport.status === 'success';
            ...
            <AppCard className="bg-black/20 p-3 rounded-lg w-full text-left mb-2 text-sm space-y-1">
                ...
                {isRouteLoading && (
                    <div className="flex items-center justify-center gap-2 p-2 text-azul-claro">
                        <Loader2 className="h-4 w-4 animate-spin" /> <span>Calculando rota...</span>
                    </div>
                )}
                {routeError && (
