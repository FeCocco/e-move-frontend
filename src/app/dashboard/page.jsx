...
const EditarUsuarioSchema = z.object({
    ...
    telefone: z.string().regex(/^\d+$/, { message: "O telefone deve conter apenas números." }).min(10, { message: "O telefone não pode ter mais de 11 dígitos." }),
});
...
export default function DashboardPage() {
    ...
    const EditarUsuarioSubmit = async (data) => { /* ... (código mantido igual) ... */ };
    const handleLogout = async () => { /* ... (código mantido igual) ... */ };
    ...
    if (!profileData) return <div className="flex justify-center items-center h-screen"><p className="text-white text-2xl">Verificando sessão...</p></div>;
                ...
                <ViagensProvider>
                    <AppCard className="h-screen sm:h-[90vh] w-full max-w-6xl p-2 sm:p-4 rounded-none sm:rounded-xl border-0 sm:border bg-transparent sm:bg-white/[0.08]" >
                        <div className="sm:hidden text-center mb-4 pt-8">
                            <Link href="/"><Logo className="text-4xl inline-block text-azul-claro/70 drop-shadow-[0_0_8px_rgba(0,255,255,0.4)]" /></Link>
                        </div>
                        <h1 className="hidden sm:block text-2xl sm:text-3xl font-bold font-orbitron text-azul-claro text-center mb-4 sm:mb-7">Painel de Controle e-Move</h1>
                        <div className="flex justify-center mb-6 sm:mb-8">
                            <DashboardNav activeTab={activeTab} setActiveTab={setActiveTab} />
                        </div>
                        <div className="flex-grow overflow-y-auto px-1 py-4 sm:p-4">
                            <div style={{ display: activeTab === '#BemVindo' ? 'block' : 'none' }}><BemVindo profileData={profileData} /></div>
                            <div style={{ display: activeTab === '#AbaVeiculos' ? 'block' : 'none' }}><AbaVeiculos /></div>
                            <div style={{ display: activeTab === '#AbaRotas' ? 'block' : 'none' }}>
                                <AbaRotas setActiveTab={setActiveTab} /> {/* PROP ADICIONADA */}
                            </div>
                            <div style={{ display: activeTab === '#AbaEstacoes' ? 'block' : 'none' }}><AbaEstacoes /></div>
                            <div style={{ display: activeTab === '#AbaMapa' ? 'block' : 'none' }}><AbaMapa isVisible={activeTab === '#AbaMapa'} /></div>
                            ...
                            <div style={{ display: activeTab === '#AbaRelatorio' ? 'block' : 'none' }}><AbaRelatorio /></div>
                        </div>
                    </AppCard>
                </ViagensProvider>
            </VeiculosProvider>
